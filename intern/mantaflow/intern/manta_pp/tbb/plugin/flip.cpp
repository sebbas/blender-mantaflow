...
void updateSndParts(LevelsetGrid& phi, FlagGrid& flags, MACGrid& vel, Vec3 gravity, BasicParticleSystem& parts, ParticleDataImpl<Vec3>& partVel, ParticleDataImpl<int>* partLife=NULL) {
	...
	for (IndexInt idx=0; idx<(int)parts.size(); idx++)
	{
		...
		if (parts.isDroplet(idx) && (phiv < -DROP_THRESH || phiv2 < -DROP_THRESH)) {
			parts.setStatus(idx, ParticleBase::PNEW | ParticleBase::PBUBBLE);
		}
		else if (parts.isBubble(idx) && (phiv > 0. || phiv2 > 0.)) {
...
void sampleSndParts(LevelsetGrid& phi, FlagGrid& flags, MACGrid& vel, BasicParticleSystem& parts, int type, Real constraint, int number=1) {
	Real dt = flags.getParent()->getDt();
	RandomStream mRand(9832);

	const Real DROP_THRESH  = 0.866;
	const Real FLOAT_THRESH = 0.866;
	...
	FOR_IJK_BND(phi, 0) {
		...
		for (int i=0; i<number; ++i) {
			...
			if (type == ParticleBase::PFLOATER) {
				// Only generate float particles at surface, slightly inside/outside fluid
				...
				// Is next particle position valid?
				if (phiv2 < -FLOAT_THRESH || phiv2 > 0.) continue;
...
} static PyObject* _W_22 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); bool noTiming = _args.getOpt<bool>("notiming", -1, 0); pbPreparePlugin(parent, "sampleSndParts" , !noTiming ); PyObject *_retval = 0; { ArgLocker _lock; LevelsetGrid& phi = *_args.getPtr<LevelsetGrid >("phi",0,&_lock); FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",1,&_lock); MACGrid& vel = *_args.getPtr<MACGrid >("vel",2,&_lock); BasicParticleSystem& parts = *_args.getPtr<BasicParticleSystem >("parts",3,&_lock); int type = _args.get<int >("type",4,&_lock); Real constraint = _args.get<Real >("constraint",5,&_lock); int number = _args.getOpt<int >("number",6,1,&_lock);   _retval = getPyNone(); sampleSndParts(phi,flags,vel,parts,type,constraint,number);  _args.check(); } pbFinalizePlugin(parent,"sampleSndParts", !noTiming ); return _retval; } catch(std::exception& e) { pbSetError("sampleSndParts",e.what()); return 0; } } static const Pb::Register _RP_sampleSndParts ("","sampleSndParts",_W_22);  extern "C" { void PbRegister_sampleSndParts() { KEEP_UNUSED(_RP_sampleSndParts); } } 
