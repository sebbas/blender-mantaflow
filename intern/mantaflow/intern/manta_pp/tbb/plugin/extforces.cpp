...
//! set no-stick wall boundary condition between ob/fl and ob/ob cells
 struct KnSetWallBcs : public KernelBase { KnSetWallBcs(const FlagGrid& flags, MACGrid& vel, const MACGrid* obvel) :  KernelBase(&flags,0) ,flags(flags),vel(vel),obvel(obvel)   { runMessage(); run(); }  inline void op(int i, int j, int k, const FlagGrid& flags, MACGrid& vel, const MACGrid* obvel ) const {
	bool curFluid = flags.isFluid(i,j,k);
...
//! add Forces between fl/fl and fl/em cells (interpolate cell centered forces to MAC grid)
 struct KnAddForceIfLower : public KernelBase { KnAddForceIfLower(const FlagGrid& flags, MACGrid& vel, const Grid<Vec3>& force) :  KernelBase(&flags,1) ,flags(flags),vel(vel),force(force)   { runMessage(); run(); }  inline void op(int i, int j, int k, const FlagGrid& flags, MACGrid& vel, const Grid<Vec3>& force ) const {
	bool curFluid = flags.isFluid(i,j,k);
	bool curEmpty = flags.isEmpty(i,j,k);
	if (!curFluid && !curEmpty) return;
...
}   inline const MACGrid& getArg0() { return source; } typedef MACGrid type0;inline MACGrid& getArg1() { return target; } typedef MACGrid type1;inline const Grid<int>* getArg2() { return count; } typedef Grid<int> type2;inline const int& getArg3() { return mode; } typedef int type3; void runMessage() { debMsg("Executing kernel KnMakeGuidingField ", 3); debMsg("Kernel range" <<  " x "<<  maxX  << " y "<< maxY  << " z "<< minZ<<" - "<< maxZ  << " "   , 4); }; void operator() (const tbb::blocked_range<IndexInt>& __r) const {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ>1) { for (int k=__r.begin(); k!=(int)__r.end(); k++) for (int j=1; j<_maxY; j++) for (int i=1; i<_maxX; i++) op(i,j,k,source,target,count,mode); } else { const int k=0; for (int j=__r.begin(); j!=(int)__r.end(); j++) for (int i=1; i<_maxX; i++) op(i,j,k,source,target,count,mode); }  } void run() {  if (maxZ>1) tbb::parallel_for (tbb::blocked_range<IndexInt>(minZ, maxZ), *this); else tbb::parallel_for (tbb::blocked_range<IndexInt>(1, maxY), *this);  }  const MACGrid& source; MACGrid& target; const Grid<int>* count; const int mode;   };
