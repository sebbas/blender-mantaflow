        ...
        for (IndexInt z = k - radius; z <= k + radius; z++) {
          ...
          Real h =
              !potTA.is3D() ?
                  1.414 * radius :
                  1.732 *
                      radius;  //estimate sqrt(2)*radius resp. sqrt(3)*radius for h, due to squared resp. cubic neighbor area
                 ...
                 const int antitunneling) const
  {
    ...
    if (neighborRatio(gridpos) < c_s) {
      ...
      v_sec[idx] +=
          dt *
          ((f_sec[idx] / 1) +
           gravity);  //TODO: if forces are added (e.g. fluid guiding), add parameter for mass instead of 1
