...
#line 1 "C:/Blender/Mantaflow/source/plugin/sndparticles.cpp"
/******************************************************************************
 *
...
void adjustSndParts(BasicParticleSystem& parts, FlagGrid& flags, LevelsetGrid& phi, ParticleDataImpl<Vec3>& partVel, ParticleDataImpl<Real>* partLife=NULL, int maxDroplet=16, int maxBubble=16, int maxFloater=16, int maxTracer=16) {
	...
	parts.doCompress();
} static PyObject* _W_0 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); bool noTiming = _args.getOpt<bool>("notiming", -1, 0); pbPreparePlugin(parent, "adjustSndParts" , !noTiming ); PyObject *_retval = 0; { ArgLocker _lock; BasicParticleSystem& parts = *_args.getPtr<BasicParticleSystem >("parts",0,&_lock); FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",1,&_lock); LevelsetGrid& phi = *_args.getPtr<LevelsetGrid >("phi",2,&_lock); ParticleDataImpl<Vec3>& partVel = *_args.getPtr<ParticleDataImpl<Vec3> >("partVel",3,&_lock); ParticleDataImpl<Real>* partLife = _args.getPtrOpt<ParticleDataImpl<Real> >("partLife",4,NULL,&_lock); int maxDroplet = _args.getOpt<int >("maxDroplet",5,16,&_lock); int maxBubble = _args.getOpt<int >("maxBubble",6,16,&_lock); int maxFloater = _args.getOpt<int >("maxFloater",7,16,&_lock); int maxTracer = _args.getOpt<int >("maxTracer",8,16,&_lock);   _retval = getPyNone(); adjustSndParts(parts,flags,phi,partVel,partLife,maxDroplet,maxBubble,maxFloater,maxTracer);  _args.check(); } pbFinalizePlugin(parent,"adjustSndParts", !noTiming ); return _retval; } catch(std::exception& e) { pbSetError("adjustSndParts",e.what()); return 0; } } static const Pb::Register _RP_adjustSndParts ("","adjustSndParts",_W_0);  extern "C" { void PbRegister_adjustSndParts() { KEEP_UNUSED(_RP_adjustSndParts); } } 

//! update velocities. set new particle position. optional: convert between particle types, partLife update



void updateSndParts(LevelsetGrid& phi, FlagGrid& flags, MACGrid& vel, Vec3 gravity, BasicParticleSystem& parts, ParticleDataImpl<Vec3>& partVel, ParticleDataImpl<Real>* partLife=NULL, Real riseBubble=0.5, Real lifeDroplet=30.0, Real lifeBubble=30.0, Real lifeFloater=30.0, Real lifeTracer=30.0) {
