    ...
    if (flags.isOutflow(i, j, k)) {
      ...
      for (int c = 0; c < dim; c++) {
        Real factor =
            ...
            max((Real)1.0,
                bulkVel
                    [c]);  // prevent the extrapolated velocity from exploding when bulk velocity below 1
        ...
        for (int d = 0; d < bWidth + 1; d++) {
          ...
          if (cur[c] < flags.getSize()[c] - d - 1 && flags.isFluid(flUp)) {
            ...
            if (done)
              velDst(i, j, k)[c] = 0.5 * (velDst(i, j, k)[c] +
                                          ((vel(i, j, k)[c] - velPrev(i, j, k)[c]) / factor) +
                                          vel(up)[c]);
            else
              velDst(i, j, k)[c] = ((vel(i, j, k)[c] - velPrev(i, j, k)[c]) / factor) + vel(up)[c];
            done = true;
          }
          flLow[c] = flLow[c] - 1;
          flUp[c] = flUp[c] + 1;
...
void applyOutflowBC(
    const FlagGrid &flags, MACGrid &vel, const MACGrid &velPrev, double timeStep, int bWidth = 1)
{
                                   ...
                                   int orderTrace)
{
  ...
  if (order == 1) {
    if (openBounds)
      applyOutflowBC(flags, fwd, orig, dt, bWidth);
    orig.swap(fwd);
  }
  else if (order == 2) {  // MacCormack
    MACGrid bwd(parent);
    ...
    if (openBounds)
      applyOutflowBC(flags, newGrid, orig, dt, bWidth);
    orig.swap(newGrid);
  }
}
