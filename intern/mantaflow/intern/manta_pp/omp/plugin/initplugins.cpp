...
#line 33 "plugin/initplugins.cpp"
...
#line 49 "plugin/initplugins.cpp"
...
#pragma omp for
        for (int k = minZ; k < maxZ; k++)
          for (int j = 0; j < _maxY; j++)
            for (int i = 0; i < _maxX; i++)
              op(i, j, k, flags, target, source, isAbsolute, type);
      }
    }
    else {
      const int k = 0;
...
void applyEmission(
    FlagGrid &flags, Grid<Real> &target, Grid<Real> &source, bool isAbsolute = true, int type = 0)
{
...
static PyObject *_W_7(PyObject *_self, PyObject *_linargs, PyObject *_kwds)
{
  try {
    ...
    PyObject *_retval = 0;
    {
      ...
      bool isAbsolute = _args.getOpt<bool>("isAbsolute", 3, true, &_lock);
      int type = _args.getOpt<int>("type", 4, 0, &_lock);
      _retval = getPyNone();
      applyEmission(flags, target, source, isAbsolute, type);
      _args.check();
    }
