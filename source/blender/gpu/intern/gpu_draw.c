...
static GPUTexture *create_field_texture(SmokeDomainSettings *sds)
{
  ...
  switch (sds->coba_field) {
    ...
    case FLUID_DOMAIN_FIELD_VELOCITY_X:
      field = fluid_get_velocity_x(sds->fluid);
      break;
    case FLUID_DOMAIN_FIELD_VELOCITY_Y:
      field = fluid_get_velocity_y(sds->fluid);
      break;
    case FLUID_DOMAIN_FIELD_VELOCITY_Z:
      field = fluid_get_velocity_z(sds->fluid);
      break;
    ...
    case FLUID_DOMAIN_FIELD_FORCE_X:
      field = fluid_get_force_x(sds->fluid);
      break;
    case FLUID_DOMAIN_FIELD_FORCE_Y:
      field = fluid_get_force_y(sds->fluid);
      break;
    case FLUID_DOMAIN_FIELD_FORCE_Z:
      field = fluid_get_force_z(sds->fluid);
      break;
    default:
      return NULL;
  }
...
static GPUTexture *create_density_texture(SmokeDomainSettings *sds, int highres)
{
  ...
  const bool has_color = (highres) ? smoke_turbulence_has_colors(sds->fluid) :
  ...
  if (highres) {
    if (has_color) {
      ...
      source = smoke_turbulence_get_density(sds->fluid);
    }
  }
  else {
    if (has_color) {
...
static GPUTexture *create_flame_texture(SmokeDomainSettings *sds, int highres)
{
  ...
  if (highres) {
    source = smoke_turbulence_get_flame(sds->fluid);
  }
  else {
...
void GPU_create_smoke_velocity(SmokeModifierData *smd)
{
  ...
  if (smd->type & MOD_SMOKE_TYPE_DOMAIN) {
    ...
    const float *vel_x = fluid_get_velocity_x(sds->fluid);
    const float *vel_y = fluid_get_velocity_y(sds->fluid);
    const float *vel_z = fluid_get_velocity_z(sds->fluid);
