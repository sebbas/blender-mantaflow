...
#line 1 "/Users/user/Developer/Xcode Projects/blenderFireIntegration/mantaflowgit/source/levelset.h"
/******************************************************************************
 *
...
void reinitMarching(FlagGrid& flags, Real maxTime=4.0, MACGrid* velTransport=NULL, bool ignoreWalls=false, bool correctOuterLayer=true, int obstacleType = FlagGrid::TypeObstacle ); static PyObject* _W_1 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); LevelsetGrid* pbo = dynamic_cast<LevelsetGrid*>(Pb::objFromPy(_self)); pbPreparePlugin(pbo->getParent(), "LevelsetGrid::reinitMarching"); PyObject *_retval = 0; { ArgLocker _lock; FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",0,&_lock); Real maxTime = _args.getOpt<Real >("maxTime",1,4.0,&_lock); MACGrid* velTransport = _args.getPtrOpt<MACGrid >("velTransport",2,NULL,&_lock); bool ignoreWalls = _args.getOpt<bool >("ignoreWalls",3,false,&_lock); bool correctOuterLayer = _args.getOpt<bool >("correctOuterLayer",4,true,&_lock); int obstacleType = _args.getOpt<int >("obstacleType",5,FlagGrid::TypeObstacle ,&_lock);  pbo->_args.copy(_args);  _retval = getPyNone(); pbo->reinitMarching(flags,maxTime,velTransport,ignoreWalls,correctOuterLayer,obstacleType);  pbo->_args.check(); } pbFinalizePlugin(pbo->getParent(),"LevelsetGrid::reinitMarching"); return _retval; } catch(std::exception& e) { pbSetError("LevelsetGrid::reinitMarching",e.what()); return 0; } }
	...
	void subtract(const LevelsetGrid& o); static PyObject* _W_4 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); LevelsetGrid* pbo = dynamic_cast<LevelsetGrid*>(Pb::objFromPy(_self)); pbPreparePlugin(pbo->getParent(), "LevelsetGrid::subtract"); PyObject *_retval = 0; { ArgLocker _lock; const LevelsetGrid& o = *_args.getPtr<LevelsetGrid >("o",0,&_lock);  pbo->_args.copy(_args);  _retval = getPyNone(); pbo->subtract(o);  pbo->_args.check(); } pbFinalizePlugin(pbo->getParent(),"LevelsetGrid::subtract"); return _retval; } catch(std::exception& e) { pbSetError("LevelsetGrid::subtract",e.what()); return 0; } }
