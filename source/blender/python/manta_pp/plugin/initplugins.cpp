...
#line 1 "/Users/user/Developer/Xcode Projects/mantaflowDevelop/mantaflowgit/source/plugin/initplugins.cpp"
/******************************************************************************
 *
...
//! Apply noise to real grid based on an SDF
 struct KnAddNoise : public KernelBase { KnAddNoise(FlagGrid& flags, Grid<Real>& density, WaveletNoiseField& noise, Grid<Real>* sdf, Real scale) :  KernelBase(&flags,0) ,flags(flags),density(density),noise(noise),sdf(sdf),scale(scale)   { run(); }  inline void op(int i, int j, int k, FlagGrid& flags, Grid<Real>& density, WaveletNoiseField& noise, Grid<Real>* sdf, Real scale )  {
	if (!flags.isFluid(i,j,k) || (sdf && (*sdf)(i,j,k) > 0.) ) return;
	density(i,j,k) += noise.evaluate(Vec3(i,j,k)) * scale;
}   inline FlagGrid& getArg0() { return flags; } typedef FlagGrid type0;inline Grid<Real>& getArg1() { return density; } typedef Grid<Real> type1;inline WaveletNoiseField& getArg2() { return noise; } typedef WaveletNoiseField type2;inline Grid<Real>* getArg3() { return sdf; } typedef Grid<Real> type3;inline Real& getArg4() { return scale; } typedef Real type4; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
...
} static PyObject* _W_1 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "addNoise" ); PyObject *_retval = 0; { ArgLocker _lock; FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",0,&_lock); Grid<Real>& density = *_args.getPtr<Grid<Real> >("density",1,&_lock); WaveletNoiseField& noise = *_args.getPtr<WaveletNoiseField >("noise",2,&_lock); Grid<Real>* sdf = _args.getPtrOpt<Grid<Real> >("sdf",3,NULL,&_lock); Real scale = _args.getOpt<Real >("scale",4,1.0 ,&_lock);   _retval = getPyNone(); addNoise(flags,density,noise,sdf,scale);  _args.check(); } pbFinalizePlugin(parent,"addNoise" ); return _retval; } catch(std::exception& e) { pbSetError("addNoise",e.what()); return 0; } } static const Pb::Register _RP_addNoise ("","addNoise",_W_1); 
...
}   inline BasicParticleSystem& getArg0() { return parts; } typedef BasicParticleSystem type0;inline ParticleDataImpl<T>& getArg1() { return pdata; } typedef ParticleDataImpl<T> type1;inline WaveletNoiseField& getArg2() { return noise; } typedef WaveletNoiseField type2;inline Real& getArg3() { return scale; } typedef Real type3; void run() {  const int _sz = size; 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
  for (int i=0; i < _sz; i++) op(i,parts,pdata,noise,scale);  }  } BasicParticleSystem& parts; ParticleDataImpl<T>& pdata; WaveletNoiseField& noise; Real scale;   };
...
// blender init functions 
 ...
 struct KnApplyEmission : public KernelBase { KnApplyEmission(FlagGrid& flags, Grid<Real>& density, Grid<Real>& emission, bool isAbsolute) :  KernelBase(&flags,0) ,flags(flags),density(density),emission(emission),isAbsolute(isAbsolute)   { run(); }  inline void op(int i, int j, int k, FlagGrid& flags, Grid<Real>& density, Grid<Real>& emission, bool isAbsolute )  {
	...
	if (isAbsolute)
		...
		density(i,j,k) += emission(i,j,k);
}   inline FlagGrid& getArg0() { return flags; } typedef FlagGrid type0;inline Grid<Real>& getArg1() { return density; } typedef Grid<Real> type1;inline Grid<Real>& getArg2() { return emission; } typedef Grid<Real> type2;inline bool& getArg3() { return isAbsolute; } typedef bool type3; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
...
void projectPpmOut( Grid<Real>& val, string name, int axis=2, Real scale=1.) {
	...
	img.writePpm( name );
} static PyObject* _W_12 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "projectPpmOut" ); PyObject *_retval = 0; { ArgLocker _lock; Grid<Real>& val = *_args.getPtr<Grid<Real> >("val",0,&_lock); string name = _args.get<string >("name",1,&_lock); int axis = _args.getOpt<int >("axis",2,2,&_lock); Real scale = _args.getOpt<Real >("scale",3,1.,&_lock);   _retval = getPyNone(); projectPpmOut(val,name,axis,scale);  _args.check(); } pbFinalizePlugin(parent,"projectPpmOut" ); return _retval; } catch(std::exception& e) { pbSetError("projectPpmOut",e.what()); return 0; } } static const Pb::Register _RP_projectPpmOut ("","projectPpmOut",_W_12); 



 struct KnPrecompLight : public KernelBase { KnPrecompLight(Grid<Real>& density, Grid<Real>& L, Vec3 light = Vec3(1,1,1) ) :  KernelBase(&density,0) ,density(density),L(L),light(light)   { run(); }  inline void op(int i, int j, int k, Grid<Real>& density, Grid<Real>& L, Vec3 light = Vec3(1,1,1)  )  {
	Vec3 n = getGradient( density, i,j,k ) * -1.; 
	normalize(n);

	Real d = dot( light, n );
	L(i,j,k) = d;
}   inline Grid<Real>& getArg0() { return density; } typedef Grid<Real> type0;inline Grid<Real>& getArg1() { return L; } typedef Grid<Real> type1;inline Vec3& getArg2() { return light; } typedef Vec3 type2; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
  for (int k=minZ; k < maxZ; k++) for (int j=0; j < _maxY; j++) for (int i=0; i < _maxX; i++) op(i,j,k,density,L,light);  } } else { const int k=0; 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
  for (int j=0; j < _maxY; j++) for (int i=0; i < _maxX; i++) op(i,j,k,density,L,light);  } }  } Grid<Real>& density; Grid<Real>& L; Vec3 light;   };
#line 257 "plugin/initplugins.cpp"
...
void projectPpmFull( Grid<Real>& val, string name, int shadeMode=0, Real scale=1.) {
	...
	img.mapRange( 1./scale );
	img.writePpm( name );
} static PyObject* _W_13 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "projectPpmFull" ); PyObject *_retval = 0; { ArgLocker _lock; Grid<Real>& val = *_args.getPtr<Grid<Real> >("val",0,&_lock); string name = _args.get<string >("name",1,&_lock); int shadeMode = _args.getOpt<int >("shadeMode",2,0,&_lock); Real scale = _args.getOpt<Real >("scale",3,1.,&_lock);   _retval = getPyNone(); projectPpmFull(val,name,shadeMode,scale);  _args.check(); } pbFinalizePlugin(parent,"projectPpmFull" ); return _retval; } catch(std::exception& e) { pbSetError("projectPpmFull",e.what()); return 0; } } static const Pb::Register _RP_projectPpmFull ("","projectPpmFull",_W_13); 
...
} static PyObject* _W_14 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "addTestParts" ); PyObject *_retval = 0; { ArgLocker _lock; BasicParticleSystem& parts = *_args.getPtr<BasicParticleSystem >("parts",0,&_lock); int num = _args.get<int >("num",1,&_lock);   _retval = getPyNone(); addTestParts(parts,num);  _args.check(); } pbFinalizePlugin(parent,"addTestParts" ); return _retval; } catch(std::exception& e) { pbSetError("addTestParts",e.what()); return 0; } } static const Pb::Register _RP_addTestParts ("","addTestParts",_W_14); 
...
}   inline Grid<Real> & getArg0() { return phiObs; } typedef Grid<Real>  type0;inline MACGrid& getArg1() { return vel; } typedef MACGrid type1;inline const Vec3& getArg2() { return center; } typedef Vec3 type2;inline const Real& getArg3() { return radius; } typedef Real type3; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
...
#line 391 "plugin/initplugins.cpp"
...
} static PyObject* _W_16 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "initVortexVelocity" ); PyObject *_retval = 0; { ArgLocker _lock; Grid<Real> & phiObs = *_args.getPtr<Grid<Real>  >("phiObs",0,&_lock); MACGrid& vel = *_args.getPtr<MACGrid >("vel",1,&_lock); const Vec3& center = _args.get<Vec3 >("center",2,&_lock); const Real& radius = _args.get<Real >("radius",3,&_lock);   _retval = getPyNone(); initVortexVelocity(phiObs,vel,center,radius);  _args.check(); } pbFinalizePlugin(parent,"initVortexVelocity" ); return _retval; } catch(std::exception& e) { pbSetError("initVortexVelocity",e.what()); return 0; } } static const Pb::Register _RP_initVortexVelocity ("","initVortexVelocity",_W_16); 
...
inline static Real calcFraction(Real phi1, Real phi2)
{
 ...
 struct KnUpdateFractions : public KernelBase { KnUpdateFractions(FlagGrid& flags, Grid<Real>& phiObs, MACGrid& fractions, const int &boundaryWidth) :  KernelBase(&flags,1) ,flags(flags),phiObs(phiObs),fractions(fractions),boundaryWidth(boundaryWidth)   { run(); }  inline void op(int i, int j, int k, FlagGrid& flags, Grid<Real>& phiObs, MACGrid& fractions, const int &boundaryWidth )  {
...
}   inline FlagGrid& getArg0() { return flags; } typedef FlagGrid type0;inline Grid<Real>& getArg1() { return phiObs; } typedef Grid<Real> type1;inline MACGrid& getArg2() { return fractions; } typedef MACGrid type2;inline const int& getArg3() { return boundaryWidth; } typedef int type3; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
...
#line 433 "plugin/initplugins.cpp"
...
} static PyObject* _W_17 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "updateFractions" ); PyObject *_retval = 0; { ArgLocker _lock; FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",0,&_lock); Grid<Real>& phiObs = *_args.getPtr<Grid<Real> >("phiObs",1,&_lock); MACGrid& fractions = *_args.getPtr<MACGrid >("fractions",2,&_lock); const int& boundaryWidth = _args.getOpt<int >("boundaryWidth",3,0,&_lock);   _retval = getPyNone(); updateFractions(flags,phiObs,fractions,boundaryWidth);  _args.check(); } pbFinalizePlugin(parent,"updateFractions" ); return _retval; } catch(std::exception& e) { pbSetError("updateFractions",e.what()); return 0; } } static const Pb::Register _RP_updateFractions ("","updateFractions",_W_17); 


 struct KnUpdateFlags : public KernelBase { KnUpdateFlags(FlagGrid& flags, MACGrid& fractions, Grid<Real>& phiObs) :  KernelBase(&flags,1) ,flags(flags),fractions(fractions),phiObs(phiObs)   { run(); }  inline void op(int i, int j, int k, FlagGrid& flags, MACGrid& fractions, Grid<Real>& phiObs )  {
...
}   inline FlagGrid& getArg0() { return flags; } typedef FlagGrid type0;inline MACGrid& getArg1() { return fractions; } typedef MACGrid type1;inline Grid<Real>& getArg2() { return phiObs; } typedef Grid<Real> type2; void run() {  const int _maxX = maxX; const int _maxY = maxY; if (maxZ > 1) { 
#pragma omp parallel 
 { this->threadId = omp_get_thread_num(); this->threadNum = omp_get_num_threads();  
#pragma omp for 
...
#line 503 "plugin/initplugins.cpp"



void setObstacleFlags(FlagGrid& flags, MACGrid& fractions, Grid<Real>& phiObs) {
	KnUpdateFlags(flags,fractions, phiObs);
} static PyObject* _W_18 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "setObstacleFlags" ); PyObject *_retval = 0; { ArgLocker _lock; FlagGrid& flags = *_args.getPtr<FlagGrid >("flags",0,&_lock); MACGrid& fractions = *_args.getPtr<MACGrid >("fractions",1,&_lock); Grid<Real>& phiObs = *_args.getPtr<Grid<Real> >("phiObs",2,&_lock);   _retval = getPyNone(); setObstacleFlags(flags,fractions,phiObs);  _args.check(); } pbFinalizePlugin(parent,"setObstacleFlags" ); return _retval; } catch(std::exception& e) { pbSetError("setObstacleFlags",e.what()); return 0; } } static const Pb::Register _RP_setObstacleFlags ("","setObstacleFlags",_W_18); 
