...
template<class T> class Grid : public GridBase {public:
	...
	void readGridFromMemory(const std::string& memLoc, const std::string gridName, int x, int y, int z); static PyObject* _W_20 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); Grid* pbo = dynamic_cast<Grid*>(Pb::objFromPy(_self)); pbPreparePlugin(pbo->getParent(), "Grid::readGridFromMemory"); PyObject *_retval = 0; { ArgLocker _lock; const std::string& memLoc = _args.get<std::string >("memLoc",0,&_lock); const std::string gridName = _args.get<std::string >("gridName",1,&_lock); int x = _args.get<int >("x",2,&_lock); int y = _args.get<int >("y",3,&_lock); int z = _args.get<int >("z",4,&_lock);  pbo->_args.copy(_args);  _retval = getPyNone(); pbo->readGridFromMemory(memLoc,gridName,x,y,z);  pbo->_args.check(); } pbFinalizePlugin(pbo->getParent(),"Grid::readGridFromMemory"); return _retval; } catch(std::exception& e) { pbSetError("Grid::readGridFromMemory",e.what()); return 0; } }
	void readAdaptiveGridFromMemory(const std::string& memLoc, const std::string gridName, Vec3i min, Vec3i max); static PyObject* _W_21 (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); Grid* pbo = dynamic_cast<Grid*>(Pb::objFromPy(_self)); pbPreparePlugin(pbo->getParent(), "Grid::readAdaptiveGridFromMemory"); PyObject *_retval = 0; { ArgLocker _lock; const std::string& memLoc = _args.get<std::string >("memLoc",0,&_lock); const std::string gridName = _args.get<std::string >("gridName",1,&_lock); Vec3i min = _args.get<Vec3i >("min",2,&_lock); Vec3i max = _args.get<Vec3i >("max",3,&_lock);  pbo->_args.copy(_args);  _retval = getPyNone(); pbo->readAdaptiveGridFromMemory(memLoc,gridName,min,max);  pbo->_args.check(); } pbFinalizePlugin(pbo->getParent(),"Grid::readAdaptiveGridFromMemory"); return _retval; } catch(std::exception& e) { pbSetError("Grid::readAdaptiveGridFromMemory",e.what()); return 0; } }
	// c++ only operators
